//
// Created by Haley Chung on 2/9/2020.
//
...
void QuadraticPtrHashTable<HashedObj>::insert( const HashedObj & x )
{
  ...
  if( isActive( currentPos ) )
    return;
  array[ currentPos ] = HashEntry( x, isActive(currentPos) ); // change made from ACTIVE
...
void QuadraticPtrHashTable<HashedObj>::rehash( )
{
  ...
  for( int j = 0; j < array.size( ); j++ )
    array[ j ].info = NULL; // changed from EMPTY

  // Copy table over
  currentSize = 0;
  for( int i = 0; i < oldArray.size( ); i++ )
    if( oldArray[ i ].info == isActive(i) ) // changed from ACTIVE
...
/* 3*/      while( array[ currentPos ].info != NULL &&
      array[ currentPos ].element != x ) // changed from EMPTY
  {
...
void QuadraticPtrHashTable<HashedObj>::remove( const HashedObj & x )
{
  int currentPos = findPos( x );
  if( isActive( currentPos ) )
    array[ currentPos ].info = ITEM_NOT_FOUND; // changed from DELETED // POINTER TO THIS OBJECT?
}
...
void QuadraticPtrHashTable<HashedObj>::makeEmpty( )
{
  ...
  for( int i = 0; i < array.size( ); i++ )
    array[ i ].info = NULL; // changed from EMPTY
}
...
{
  return array[ currentPos ].info == isActive(currentPos); // changed from ACTIVE
}
