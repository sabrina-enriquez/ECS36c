...
template<class Comparable>
bool BinarySearchTree<Comparable>::printRange(const Comparable& x, const Comparable& y, BinaryNode<Comparable>* t) const {
  if (x < y) {
    if (t != NULL) {
      if (t->element >= x && t->left != NULL) {
        printRange(x, y, t->left);
      }
      if (t->element > y) {
        return true;
      }
      if (t->element <= y && t->element >= x) {
        std::cout << t->element << " ";
      }
      if (t->element <= y && t->right != NULL) {
        printRange(x, y, t->right);
      }
    }
  }
  else if ( y < x) {
    if (t != NULL) {
      if (t->element >= y && t->left != NULL) {
        printRange(x, y, t->left);
